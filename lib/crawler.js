...
const crawler = (url, data) => {
    return new Promise(async (resolve, reject) => {
        if (Object.prototype.toString.call(data) === "[object Object]") {
            const scope = await generateScopping(data)
            runXray(url, scope)
...
const domCrawler = (url, data, paginate, paginateLimit = 1, paginateClick, paginateEnd, clickBefore) => {
    return new Promise(async (resolve, reject) => {
        const html = await getDOMHTML(url, paginate, paginateLimit, paginateClick, paginateEnd, clickBefore)
        if (paginate) {
            let objs = []
            if (Object.prototype.toString.call(data) === "[object Object]") {
                const scope = await generateScopping(data)
...
const getDOMHTML = async (url, paginate, paginateLimit = 1, paginateClick, paginateEnd, clickBefore) => {
    const browser = await puppeteer.launch({headless: true})
    const [page] = await browser.pages()
    await page.setViewport({ width: 1366, height: 768})
    await page.goto(url, {waitUntil: 'networkidle0'})
    await page.waitForSelector('html')
    clickBefore.map(async (elm) => {
